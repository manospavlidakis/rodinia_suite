...
extern "C" void allocateMemory(int npoints, int nfeatures, int nclusters,
  ...
  cudaMemcpy(feature_flipped_d, features[0],
             npoints * nfeatures * sizeof(float), cudaMemcpyHostToDevice);
  ...
  cudaMalloc((void **)&block_clusters_d, num_blocks_perdim * num_blocks_perdim *
                                             nclusters * nfeatures *
                                             sizeof(float));
  // cudaMemcpy(new_clusters_d, new_centers[0],
  // nclusters*nfeatures*sizeof(float), cudaMemcpyHostToDevice);
#endif
}
/* -------------- allocateMemory() end ------------------- */
...
kmeansCuda(float **feature,      /* in: [npoints][nfeatures] */
  ...
  cudaMemcpy(membership_d, membership_new, npoints * sizeof(int),
  ...
  cudaMemcpyToSymbol("c_clusters", clusters[0],
    ...
    for (int j = 0; j < nclusters; j++) {
      for (int k = 0; k < nfeatures; k++) {
        block_new_centers[j * nfeatures + k] +=
            block_clusters_h[i * nclusters * nfeatures + j * nfeatures + k];
      }
    }
  }
  ...
  block_new_centers[j*nfeatures + k]) { printf("\t \t for %d:%d, normal value is
  %e and gpu reduced value id %e
