...
cudaErrorCheckFatal_Runtime(hipError_t err, const char *func, const char *file,
                            size_t line) {
  if (err != hipSuccess) {
...
int main(int argc, char *argv[]) {
  ...
  // Warmup
  double *warm;
  HIP_CHECK(hipMalloc((void **)&warm, sizeof(double) * 1000000));
  hipStream_t stream;
...
void PrintDeviceProperties() {
  ...
  HIP_CHECK(hipGetDeviceCount(&nDevCount));
...
void ForwardSub() {
  ...
  HIP_CHECK(hipMalloc((void **)&m_cuda, Size * Size * sizeof(float)));
  HIP_CHECK(hipMalloc((void **)&a_cuda, Size * Size * sizeof(float)));
  HIP_CHECK(hipMalloc((void **)&b_cuda, Size * sizeof(float)));
  ...
  // copy memory to GPU
  HIP_CHECK(hipMemcpy(m_cuda, m, Size * Size * sizeof(float),
                                      hipMemcpyHostToDevice));
  HIP_CHECK(hipMemcpy(a_cuda, a, Size * Size * sizeof(float),
                                      hipMemcpyHostToDevice));
  HIP_CHECK(hipMemcpy(b_cuda, b, Size * sizeof(float), hipMemcpyHostToDevice));
  ...
  HIP_CHECK(hipMemcpy(m, m_cuda, Size * Size * sizeof(float),
                                      hipMemcpyDeviceToHost));
  HIP_CHECK(hipMemcpy(a, a_cuda, Size * Size * sizeof(float),
                                      hipMemcpyDeviceToHost));
  HIP_CHECK(hipMemcpy(b, b_cuda, Size * sizeof(float), hipMemcpyDeviceToHost));
  ...
  HIP_CHECK(hipFree(m_cuda));
  HIP_CHECK(hipFree(a_cuda));
  HIP_CHECK(hipFree(b_cuda));
