...
#include "../../common/my_verification.h"
#include <chrono>
#include <fstream>
#include <iostream>
...
template <typename T> T *alloc(int N) {
  T *t;
  HIP_CHECK(hipMalloc((void **)&t, sizeof(T) * N));
  return t;
}

template <typename T> void dealloc(T *array) {
  HIP_CHECK(hipFree((void *)array));
}

template <typename T> void copy(T *dst, T *src, int N) {
  HIP_CHECK(hipMemcpy((void *)dst, (void *)src, N * sizeof(T),
...
int main(int argc, char **argv) {
  ...
  HIP_CHECK(hipMemcpyToSymbol(HIP_SYMBOL(ff_flux_contribution_momentum_x),
                                     &h_ff_flux_contribution_momentum_x,
                                     sizeof(float3)));
  HIP_CHECK(hipMemcpyToSymbol(HIP_SYMBOL(ff_flux_contribution_momentum_y),
                                     &h_ff_flux_contribution_momentum_y,
                                     sizeof(float3)));
  HIP_CHECK(hipMemcpyToSymbol(HIP_SYMBOL(ff_flux_contribution_momentum_z),
                                     &h_ff_flux_contribution_momentum_z,
                                     sizeof(float3)));

  HIP_CHECK(hipMemcpyToSymbol(HIP_SYMBOL(ff_flux_contribution_density_energy),
  ...
  HIP_CHECK(hipMemset((void *)step_factors, 0, sizeof(float) * nelr));
