...
void runVLCTest(char *file_name, uint num_block_threads, uint num_blocks) {
  ...
  HIP_CHECK(hipMalloc((void **)&d_codewords, NUM_SYMBOLS * symbol_type_size));
  HIP_CHECK(hipMalloc((void **)&d_codewordlens, NUM_SYMBOLS * symbol_type_size));

  HIP_CHECK(hipMalloc((void **)&d_cw32, mem_size));
  HIP_CHECK(hipMalloc((void **)&d_cw32len, mem_size));
  HIP_CHECK(hipMalloc((void **)&d_cw32idx, mem_size));
  ...
  HIP_CHECK(hipMemcpy(d_codewordlens, codewordlens,
                            NUM_SYMBOLS * symbol_type_size,
                            hipMemcpyHostToDevice));
  ...
  HIP_CHECK(hipFree(d_codewords));
  HIP_CHECK(hipFree(d_codewordlens));
  HIP_CHECK(hipFree(d_cw32));
  HIP_CHECK(hipFree(d_cw32len));
