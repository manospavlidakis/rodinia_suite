...
int runHisto(char *file, unsigned int *freq, unsigned int memSize,
  ...
  HIP_CHECK(hipMalloc((void **)&dev_buffer0, partSize));
  HIP_CHECK(hipMalloc((void **)&dev_buffer1, partSize));
  HIP_CHECK(hipMalloc((void **)&dev_histo, 256 * sizeof(int)));
  HIP_CHECK(hipMemset(dev_histo, 0, 256 * sizeof(int)));
  ...
  for (int i = 0; i < totalNum; i += partialNum * 2) {
    ...
    HIP_CHECK(hipMemcpyAsync(dev_buffer0, buffer + i, partSize,
                          hipMemcpyHostToDevice, stream0));
    HIP_CHECK(hipMemcpyAsync(dev_buffer1, buffer + i + partialNum, partSize,
  ...
  HIP_CHECK(hipMemcpy(freq, dev_histo, 256 * sizeof(int), hipMemcpyDeviceToHost));
  HIP_CHECK(hipEventRecord(stop, 0));
  HIP_CHECK(hipEventSynchronize(stop));
  float elapsedTime;
  HIP_CHECK(hipEventElapsedTime(&elapsedTime, start, stop));

  HIP_CHECK(hipFree(dev_histo));
  HIP_CHECK(hipFree(dev_buffer0));
  HIP_CHECK(hipFree(dev_buffer1));
  return 0;
}
