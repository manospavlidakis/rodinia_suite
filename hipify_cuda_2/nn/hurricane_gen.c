...
int main(int argc, char **argv)
{
  ...
  if(argc < 3)
  {
    FPRINTF_CHECK(stderr,"we need a number of hurricanes to create and number of files to create\n");
    exit(0);
  }
  ...
  {
    char fname[64];  /* make it bigger than 16 */
    SNPRINTF_CHECKA(fname, sizeof(fname), "data/cane%d_%d.db", num_files, j);

    if ((fp = fopen(fname, "w")) == NULL) {
		 FPRINTF_CHECK(stderr, "Failed to open output file '%s'!\n", fname);
		 return -1;
	 }
    ...
    for(i=0;i<canes;i++)
    {
      ...
      FPRINTF_CHECK(fp, "%4d %2d %2d %2d %2d %-9s %5.1f %5.1f %4d %4d\n", year, month, date, hour, num, name, lat, lon, speed, press);
