...
void cuda_print_double_array(double *array_GPU, size_t size) {
  ...
  HIP_CHECK(hipMemcpy(mem, array_GPU, sizeof(double) * size, hipMemcpyDeviceToHost));
 ...
 ******************************/
__forceinline__ __device__ double d_randu(int *seed, int index) {
...
__forceinline__ __device__ double d_randn(int *seed, int index) {
  // Box-Muller algortihm
  double pi = 3.14159265358979323846;
 ...
 * > input value
 */
__forceinline__ __device__ double dev_round_double(double value) {
...
void videoSequence(unsigned char *I, int IszX, int IszY, int Nfr, int *seed) {
  ...
  /*dilate matrix*/
  //    unsigned char * newMatrix = (unsigned char *) malloc(sizeof (unsigned
  //    char) * IszX * IszY * Nfr);
...
particleFilter(unsigned char *I, int IszX, int IszY, int Nfr, int *seed,
  ...
  HIP_CHECK(hipMalloc((void **)&arrayX_GPU, sizeof(double) * Nparticles));
  HIP_CHECK(hipMalloc((void **)&arrayY_GPU, sizeof(double) * Nparticles));
  HIP_CHECK(hipMalloc((void **)&xj_GPU, sizeof(double) * Nparticles));
  HIP_CHECK(hipMalloc((void **)&yj_GPU, sizeof(double) * Nparticles));
  HIP_CHECK(hipMalloc((void **)&CDF_GPU, sizeof(double) * Nparticles));
  HIP_CHECK(hipMalloc((void **)&u_GPU, sizeof(double) * Nparticles));
  HIP_CHECK(hipMalloc((void **)&likelihood_GPU, sizeof(double) * Nparticles));
  // set likelihood to zero
  HIP_CHECK(hipMemset((void *)likelihood_GPU, 0, sizeof(double) * Nparticles));
  HIP_CHECK(hipMalloc((void **)&weights_GPU, sizeof(double) * Nparticles));
  HIP_CHECK(hipMalloc((void **)&I_GPU, sizeof(unsigned char) * IszX * IszY * Nfr));
  HIP_CHECK(hipMalloc((void **)&objxy_GPU, sizeof(int) * 2 * countOnes));
  HIP_CHECK(hipMalloc((void **)&ind_GPU, sizeof(int) * countOnes * Nparticles));
  HIP_CHECK(hipMalloc((void **)&seed_GPU, sizeof(int) * Nparticles));
  HIP_CHECK(hipMalloc((void **)&partial_sums, sizeof(double) * Nparticles));
  ...
  int k;
  // start send
  HIP_CHECK(hipMemcpy(I_GPU, I, sizeof(unsigned char) * IszX * IszY * Nfr,
                         hipMemcpyHostToDevice));
  HIP_CHECK(hipMemcpy(objxy_GPU, objxy, sizeof(int) * 2 * countOnes,
                         hipMemcpyHostToDevice));
  HIP_CHECK(hipMemcpy(weights_GPU, weights, sizeof(double) * Nparticles,
                         hipMemcpyHostToDevice));
  HIP_CHECK(hipMemcpy(xj_GPU, xj, sizeof(double) * Nparticles,
                         hipMemcpyHostToDevice));
  HIP_CHECK(hipMemcpy(yj_GPU, yj, sizeof(double) * Nparticles,
                         hipMemcpyHostToDevice));
  HIP_CHECK(hipMemcpy(seed_GPU, seed, sizeof(int) * Nparticles,
                         hipMemcpyHostToDevice));
  int num_blocks = ceil((double)Nparticles / (double)threads_per_block);
  ...
  HIP_CHECK(hipFree(xj_GPU));
  HIP_CHECK(hipFree(yj_GPU));
  HIP_CHECK(hipFree(CDF_GPU));
  HIP_CHECK(hipFree(u_GPU));
  HIP_CHECK(hipFree(likelihood_GPU));
  HIP_CHECK(hipFree(I_GPU));
  HIP_CHECK(hipFree(objxy_GPU));
  HIP_CHECK(hipFree(ind_GPU));
  HIP_CHECK(hipFree(seed_GPU));
  HIP_CHECK(hipFree(partial_sums));

  HIP_CHECK(hipMemcpy(arrayX, arrayX_GPU, sizeof(double) * Nparticles,
                         hipMemcpyDeviceToHost));
  HIP_CHECK(hipMemcpy(arrayY, arrayY_GPU, sizeof(double) * Nparticles,
                         hipMemcpyDeviceToHost));
  HIP_CHECK(hipMemcpy(weights, weights_GPU, sizeof(double) * Nparticles,
                         hipMemcpyDeviceToHost));

  // CUDA freeing of memory
  HIP_CHECK(hipFree(weights_GPU));
  HIP_CHECK(hipFree(arrayY_GPU));
  HIP_CHECK(hipFree(arrayX_GPU));
...
int main(int argc, char *argv[]) {
  ...
  // Warmup
  char *warm;
  HIP_CHECK(hipMalloc((void **)&warm, sizeof(char)));
  hipStream_t stream;
