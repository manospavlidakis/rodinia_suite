...
#include "kernel.cu.hip"
#include "kernel2.cu.hip"
...
void BFSGraph(int argc, char **argv) {
  ...
  for (unsigned int i = 0; i < no_of_nodes; i++) {
    FSCANF_CHECK(fp, "%d %d", &start, &edgeno);
  ...
  FSCANF_CHECK(fp, "%d", &edge_list_size);

  int id, cost;
  int *h_graph_edges = (int *)malloc(sizeof(int) * edge_list_size);
  for (int i = 0; i < edge_list_size; i++) {
    FSCANF_CHECK(fp, "%d", &id);
    FSCANF_CHECK(fp, "%d", &cost);
    h_graph_edges[i] = id;
  }
  ...
  Node *d_graph_nodes;
  HIP_CHECK(hipMalloc((void **)&d_graph_nodes, sizeof(Node) * no_of_nodes));
  // Copy the Edge List to device Memory
  int *d_graph_edges;
  HIP_CHECK(hipMalloc((void **)&d_graph_edges, sizeof(int) * edge_list_size));

  // Copy the Mask to device memory
  bool *d_graph_mask;
  HIP_CHECK(hipMalloc((void **)&d_graph_mask, sizeof(bool) * no_of_nodes));

  bool *d_updating_graph_mask;
  HIP_CHECK(hipMalloc((void **)&d_updating_graph_mask, sizeof(bool) * no_of_nodes));

  // Copy the Visited nodes array to device memory
  bool *d_graph_visited;
  HIP_CHECK(hipMalloc((void **)&d_graph_visited, sizeof(bool) * no_of_nodes));

  // allocate device memory for result
  int *d_cost;
  HIP_CHECK(hipMalloc((void **)&d_cost, sizeof(int) * no_of_nodes));

  // make a bool to check if the execution is over
  bool *d_over;
  HIP_CHECK(hipMalloc((void **)&d_over, sizeof(bool)));
...
#endif
  HIP_CHECK(hipMemcpy(d_graph_nodes, h_graph_nodes, sizeof(Node) * no_of_nodes,
             hipMemcpyHostToDevice));

  HIP_CHECK(hipMemcpy(d_graph_edges, h_graph_edges, sizeof(int) * edge_list_size,
             hipMemcpyHostToDevice));

  HIP_CHECK(hipMemcpy(d_graph_mask, h_graph_mask, sizeof(bool) * no_of_nodes,
             hipMemcpyHostToDevice));

  HIP_CHECK(hipMemcpy(d_updating_graph_mask, h_updating_graph_mask,
             sizeof(bool) * no_of_nodes, hipMemcpyHostToDevice));

  HIP_CHECK(hipMemcpy(d_graph_visited, h_graph_visited, sizeof(bool) * no_of_nodes,
             hipMemcpyHostToDevice));

  HIP_CHECK(hipMemcpy(d_cost, h_cost, sizeof(int) * no_of_nodes, hipMemcpyHostToDevice));
#ifdef BREAKDOWNS
  ...
  do {
    ...
    stop = false;
    HIP_CHECK(hipMemcpy(d_over, &stop, sizeof(bool), hipMemcpyHostToDevice));
    ...
    HIP_CHECK(hipMemcpy(&stop, d_over, sizeof(bool), hipMemcpyDeviceToHost));
    k++;
  } while (stop);
#ifdef BREAKDOWNS
  HIP_CHECK(hipDeviceSynchronize());
  ...
  HIP_CHECK(hipFree(d_graph_mask));
