...
int main(int argc, char *argv[]) {
  ...
  HIP_CHECK(hipMemcpy(common.d_endoRow, common.endoRow, common.endo_mem,
  ...
  HIP_CHECK(hipMemcpy(common.d_endoCol, common.endoCol, common.endo_mem,
            hipMemcpyHostToDevice));
  ...
  HIP_CHECK(hipMemcpy(common.d_epiRow, common.epiRow, common.epi_mem,
            hipMemcpyHostToDevice));
  ...
  HIP_CHECK(hipMemcpy(common.d_epiCol, common.epiCol, common.epi_mem,
            hipMemcpyHostToDevice));
  ...
  // pointers
  for (i = 0; i < common.allPoints; i++) {
    HIP_CHECK(hipMalloc((void **)&unique[i].d_in2, common.in2_mem));
  }
  ...
  // pointers
  for (i = 0; i < common.allPoints; i++) {
    HIP_CHECK(hipMalloc((void **)&unique[i].d_conv, common.conv_mem));
  }
  ...
  // pointers
  for (i = 0; i < common.allPoints; i++) {
    HIP_CHECK(hipMalloc((void **)&unique[i].d_in2_pad_cumv, common.in2_pad_cumv_mem));
  }
  ...
  // pointers
  for (i = 0; i < common.allPoints; i++) {
    HIP_CHECK(hipMalloc((void **)&unique[i].d_in2_pad_cumv_sel,
  ...
  // pointers
  for (i = 0; i < common.allPoints; i++) {
    HIP_CHECK(hipMalloc((void **)&unique[i].d_in2_sub_cumh, common.in2_sub_cumh_mem));
  }
  ...
  // pointers
  for (i = 0; i < common.allPoints; i++) {
    HIP_CHECK(hipMalloc((void **)&unique[i].d_in2_sub_cumh_sel,
  ...
  // pointers
  for (i = 0; i < common.allPoints; i++) {
    HIP_CHECK(hipMalloc((void **)&unique[i].d_in2_sub2, common.in2_sub2_mem));
  }
  ...
  // pointers
  for (i = 0; i < common.allPoints; i++) {
    HIP_CHECK(hipMalloc((void **)&unique[i].d_in2_sqr, common.in2_sqr_mem));
  }
  // common
  ...
  // pointers
  for (i = 0; i < common.allPoints; i++) {
    HIP_CHECK(hipMalloc((void **)&unique[i].d_in2_sqr_sub2, common.in2_sqr_sub2_mem));
  }
  ...
  // pointers
  for (i = 0; i < common.allPoints; i++) {
    HIP_CHECK(hipMalloc((void **)&unique[i].d_in_sqr, common.in_sqr_mem));
  }
  ...
  // pointers
  for (i = 0; i < common.allPoints; i++) {
    HIP_CHECK(hipMalloc((void **)&unique[i].d_tMask, common.tMask_mem));
  }
  ...
  // pointers
  for (i = 0; i < common.allPoints; i++) {
    HIP_CHECK(hipMalloc((void **)&unique[i].d_mask_conv, common.mask_conv_mem));
  }
  ...
  HIP_CHECK(hipMemcpyToSymbol(HIP_SYMBOL(d_unique), &unique,
  ...
  for (common_change.frame_no = 0; common_change.frame_no < frames_processed;
    ...
    HIP_CHECK(hipMemcpy(common_change.d_frame, frame, common.frame_mem,
              hipMemcpyHostToDevice));
    HIP_CHECK(hipMemcpyToSymbol(HIP_SYMBOL(d_common_change), &common_change,
  ...
  std::cerr << "Warmup time: " << elapsed_milli_warmup.count() << " ms"
            << std::endl;
#endif
  write_data((char*)"result.txt", common.no_frames, frames_processed,
