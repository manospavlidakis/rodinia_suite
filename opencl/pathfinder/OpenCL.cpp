...
void OpenCL::launch(string toLaunch, int version) {
  if (is_ndrange_kernel(version)) {
    CL_SAFE_CALL(clEnqueueNDRangeKernel(command_queue, kernelArray[toLaunch], 1,
                                        NULL, &gwsize, &lwsize, 0, NULL, NULL));
  } else {
    CL_SAFE_CALL(
        clEnqueueTask(command_queue, kernelArray[toLaunch], 0, NULL, NULL));
  }
}

void OpenCL::gwSize(size_t theSize) { this->gwsize = theSize; }

void OpenCL::lwSize(size_t theSize) { this->lwsize = theSize; }
...
void OpenCL::createKernel(string kernelName) {
  ...
  /*#if !defined(ALTERA)
          CL_SAFE_CALL(clGetKernelWorkGroupInfo(kernelArray[kernelName],
  device_id[0], CL_KERNEL_WORK_GROUP_SIZE, sizeof(size_t), &lwsize, NULL)); if
  (lwsize == 0)
          {
                  cout << "Error: clGetKernelWorkGroupInfo() returned a max work
  group size of zero!" << endl; exit(1);
          }
          ...
          else
          {
                  ...
                  if (VERBOSE)
                          printf("Max local threads is %zu. Using %zu for local
...
void OpenCL::buildKernel(int version) {
  ...
  char *kernel_file_path =
      getVersionedKernelName("./pathfinder_kernel", version);
  char *source = read_kernel(kernel_file_path, &size);

  // Create a program from the kernel source.
#ifdef USE_JIT
  program =
      clCreateProgramWithSource(context, 1, (const char **)&source, NULL, &ret);
#else
  program = clCreateProgramWithBinary(
      context, 1, devices, &size, (const unsigned char **)&source, NULL, &ret);
  ...
  NULL, &log_size); build_log = new char[log_size + 1];
          // Second call to get the log
          clGetProgramBuildInfo(program, device_id[0], CL_PROGRAM_BUILD_LOG,
  log_size, build_log, NULL); build_log[log_size] = '\0'; cout << build_log <<
  endl; delete[] build_log; cout <<
  "\n*************************************************" << endl; cout << "***
  END OUTPUT FROM COMPILING THE KERNEL FILE ***" << endl; cout <<
  "*************************************************\n\n" << endl; exit(1);
  }
