...
int total_kernels = 4;
string kernel_names[4] = {"likelihood_kernel", "sum_kernel",
                          "normalize_weights_kernel", "find_index_kernel"};
...
void _clCmdParams(int argc, char *argv[]) {
  for (int i = 0; i < argc; ++i) {
    ...
    case 'g': //--g stands for size of work group
      if (++i < argc) {
        // Altera's ARM compiler considers size_t as unsigned while other
        // compilers say it is unsigned long
#ifdef ARM
...
void _clInit() {
  ...
  if ((resultCL != CL_SUCCESS) || (oclHandles.context == NULL))
    throw(
        string("InitCL()::Error: Creating Context (clCreateContextFromType)"));
  //-----------------------------------------------
  //--cambine-3: detect OpenCL devices

  // First, get the size of device list
  oclHandles.cl_status =
      clGetContextInfo(oclHandles.context, CL_CONTEXT_DEVICES, 0, NULL, &size);
  ...
  oclHandles.queue = clCreateCommandQueue(oclHandles.context,
                                          ...
                                          CL_QUEUE_PROFILING_ENABLE, &resultCL);
  ...
  size_t sourceSize;
  char *kernel_file_path = getVersionedKernelName("./particle_double", 0);
  char *source = read_kernel(kernel_file_path, &sourceSize);

  // Create the program object
#ifdef USE_JIT
  oclHandles.program = clCreateProgramWithSource(
      oclHandles.context, 1, (const char **)&source, NULL, &resultCL);
#else
  ...
  clBuildProgram_SAFE(oclHandles.program, deviceListSize, oclHandles.devices,
...
void _clInvokeKernel(int kernel_id, size_t work_items, size_t work_group_size,
                     cl_event *kernel_event, int version) throw(string) {
  ...
  if (is_ndrange_kernel(version)) {
    CL_SAFE_CALL(clEnqueueNDRangeKernel(
        ...
        global_work_size, local_work_size, 0, 0, kernel_event));
  } else {
    CL_SAFE_CALL(clEnqueueTask(oclHandles.queue, oclHandles.kernel[kernel_id],
                               0, NULL, kernel_event));
  }
...
/*void _clInvokeKernel2D(int kernel_id, size_t range_x, size_t range_y, size_t
group_x, size_t group_y) throw(string){ cl_uint work_dim = WORK_DIM; size_t
local_work_size[] = {group_x, group_y}; size_t global_work_size[] = {range_x,
range_y}; cl_event e[1];
//if(work_items%work_group_size != 0)	//process situations that work_items
cannot be divided by work_group_size
...
{
#ifdef ERRMSG
CL_SAFE_CALL( clEnqueueNDRangeKernel(oclHandles.queue,
oclHandles.kernel[kernel_id], work_dim, 0, global_work_size, local_work_size, 0
, 0, &(e[0])) ); #else clEnqueueNDRangeKernel(oclHandles.queue,
oclHandles.kernel[kernel_id], work_dim, 0, global_work_size, local_work_size, 0
, 0, &(e[0])); #endif
}
else
{
#ifdef ERRMSG
CL_SAFE_CALL( clEnqueueTask(oclHandles.queue, oclHandles.kernel[kernel_id], 0,
NULL, &(e[0])) ); #else clEnqueueTask(oclHandles.queue,
oclHandles.kernel[kernel_id], 0, NULL, &(e[0])); #endif
}
