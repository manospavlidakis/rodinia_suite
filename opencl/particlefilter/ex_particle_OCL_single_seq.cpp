...
void ocl_print_float_array(cl_command_queue cmd_q, cl_mem array_GPU,
  ...
  cl_int err = clEnqueueReadBuffer(cmd_q, array_GPU, 1, 0, sizeof(float) * size,
                                   mem, 0, 0, 0);
  if (err != CL_SUCCESS) {
...
int particleFilter(unsigned char *I, int IszX, int IszY, int Nfr, int *seed,
  ...
  err =
      clBuildProgram(prog, 1, device_list, "-cl-fast-relaxed-math", NULL, NULL);
  ...
  //     device_id, NULL); if (err != CL_SUCCESS) {
  ...
  if (err != CL_SUCCESS) {
    if (err == CL_INVALID_PROGRAM)
      printf(
          "ERROR: clCreateKernel(likelihood_kernel) 0 => INVALID PROGRAM %d\n",
          err);
    if (err == CL_INVALID_PROGRAM_EXECUTABLE)
      printf("ERROR: clCreateKernel(likelihood_kernel) 0 => INVALID PROGRAM "
             "EXECUTABLE %d\n",
             err);
    if (err == CL_INVALID_KERNEL_NAME)
      printf("ERROR: clCreateKernel(likelihood_kernel) 0 => INVALID KERNEL "
             "NAME %d\n",
             err);
  ...
  u_GPU = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(float) * Nparticles,
                         NULL, &err);
  if (err != CL_SUCCESS) {
  ...
  I_GPU = clCreateBuffer(context, CL_MEM_READ_WRITE,
  ...
  for (k = 1; k < Nfr; k++) {
    ...
    // KERNEL FUNCTION CALL
    err = clEnqueueNDRangeKernel(cmd_queue, kernel_sum, 1, NULL, global_work,
                                 local_work, 0, 0, 0);
    clFinish(cmd_queue);
    if (err != CL_SUCCESS) {
    ...
    // KERNEL FUNCTION CALL
    err = clEnqueueNDRangeKernel(cmd_queue, kernel_normalize_weights, 1, NULL,
                                 global_work, local_work, 0, 0, 0);
    clFinish(cmd_queue);
    if (err != CL_SUCCESS) {
