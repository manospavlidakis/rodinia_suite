...
function parser(){
  ...
  do
    ...
    cat AVG_fpga_${work}_concurrency_${concurrency}_${str}.final >> ../${str}_fpga.merged 
  ...
  tr "\n" "," < ../${str}_fpga.merged &> ../${str}_fpga.trans
  tr "\n" "," < ../${str}_compute.merged &> ../${str}_compute.trans
  paste -d "\n" ../${str}_fpga.trans ../${str}_init.trans ../${str}_compute.trans \
  ../${str}_total.trans &> ../../${str}_all.csv
  rm -rf ../${str}_*.trans 
}
if [ $# -eq 0 ]
then
...
do
    ...
    then 
      echo "NN"
      workloads=("64k" "128k" "256k" "512k" "1024k" "2048k")
      parser ${workloads} ${concurrency} ${str}
    elif [[ "$str" == "nw" ]]
    then 
      echo "NW"
